...
const CartSlider = () => {
  ...
  useEffect(() => {
    ...
    // Clean up when the component unmounts or the cart is closed
    return () => {
  ...
  return (
    <div className="w-full h-full bg-black/35 absolute top-0 z-10">
      <div className="h-full lg:w-[65%] md:w-[50%]" onClick={() => dispatch(toggleCart())}></div>
      <motion.div
        initial={{ x: "100%" }}
        animate={{ x: 0 }}
        exit={{ x: "-100%" }}
        transition={{ duration: 0.5, ease: "easeInOut" }}
        className="lg:w-[35%] md:w-[50%] sm:w-full max-sm:w-full bg-white h-full right-0 top-0 absolute rounded-l-lg shadow-lg"
      >
        {/* cart header */}
